...
public class GeneroController {
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<Object> deletarGenero(@PathVariable Long id) {
		Optional<Genero> generoOptional = service.findById(id);
		if (!generoOptional.isPresent()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Gênero não encontrado.");
		}
	...
	@PutMapping("/{id}")
	public ResponseEntity<Object> atualizar(@PathVariable Long id,@RequestBody GeneroDto dto) {
		Optional<Genero> generoOptional = service.findById(id);
		if (!generoOptional.isPresent()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Gênero não encontrado.");
		}
		...
		genero.setDataRegistro(generoOptional.get().getDataRegistro());
		return ResponseEntity.status(HttpStatus.OK).body(service.save(genero));
	}
}
